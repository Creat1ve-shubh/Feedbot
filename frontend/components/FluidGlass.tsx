...
function Lens({ modeProps, ...p }: { modeProps?: ModeProps } & MeshProps) {
  return <ModelSwitch kind="lens" modeProps={modeProps} {...p} />;
}

function Cube({ modeProps, ...p }: { modeProps?: ModeProps } & MeshProps) {
  return <ModelSwitch kind="cube" modeProps={modeProps} {...p} />;
}
...
function PrimitiveGlass({
  shape = "cylinder",
  modeProps = {},
  ...props
}: { shape?: "cylinder" | "box"; modeProps?: ModeProps } & MeshProps) {
  const ref = useRef<THREE.Mesh>(null!);
  const {
    scale,
    ior,
...
function ModelSwitch({
  ...
  modeProps = {},
  ...p
}: { kind: "lens" | "cube" | "bar"; modeProps?: ModeProps } & MeshProps) {
  ...
  const glb =
    kind === "lens"
      ...
      : kind === "cube"
        ? "/assets/3d/cube.glb"
        : "/assets/3d/bar.glb";
  const geometryKey = kind === "lens" ? "Cylinder" : "Cube";
  const fallbackShape = kind === "lens" ? "cylinder" : "box";

  useEffect(() => {
    let mounted = true;
    fetch(glb, { method: "HEAD" })
      .then((res) => {
        if (mounted) setHasModel(res.ok);
      })
      .catch(() => {
        if (mounted) setHasModel(false);
      });
    return () => {
      mounted = false;
    };
  ...
  return (
    <PrimitiveGlass
      shape={fallbackShape as "cylinder" | "box"}
      modeProps={modeProps}
      {...p}
    />
  );
